  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 min-h-screen bg-transparent text-slate-800 dark:text-slate-100 transition-colors duration-300">
      <div className="max-w-5xl mx-auto">
        <div className="mb-8">
          <h1 className="text-2xl sm:text-3xl font-black">User Account Moderation</h1>
          <p className="text-xs text-slate-400 mt-1">Audit active candidate credentials, recruiter profiles, and delete inactive profiles</p>
        </div>
        ...
        <div className="glass-card rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-800">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-900/40 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                ...
                {users.length === 0 ? (
                  ...
                  users.map((u, idx) => {
                    ...
                    const getRoleBadge = (role) => {
                      ...
                      if (role === 'recruiter') return 'bg-purple-500/10 text-purple-400 border-purple-500/20';
                    ...
                    return (
                      <tr key={idx} className="hover:bg-slate-900/10 transition-colors">
                        ...
                        <td className="py-4.5 px-6 text-slate-450">{joined}</td>
                        <td className="py-4.5 px-6 text-center">
                          {u.role === 'admin' ? (
                            <span className="text-[10px] font-bold text-slate-550 flex items-center justify-center gap-1.5">
                              <ShieldCheck className="w-4 h-4 text-slate-500" /> Protected
                            </span>
                          ) : (
                            <button
                              onClick={() => handleDeleteUser(u._id, u.name, u.role)}
                              className="p-2 text-slate-500 hover:text-red-400 hover:bg-slate-800/40 rounded-xl transition-colors inline-flex"
                            >
